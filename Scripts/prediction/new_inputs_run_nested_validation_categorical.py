...
#for inputs in ['ROIs age corrected']:
	print(inputs)
	if inputs == 'ROIs age corrected':
		...
		random_df = pd.read_csv(os.path.join(path_to_random, 'random_parcellation_{}_FA_median_minAD.csv'.format(inputs)), index_col=0)
		df = pd.merge(random_df, df[['subject_id', 'PMA_scan','Cognitive Score_cat','Language Score_cat','Motor Score_cat','group_cat', 'qc_translation', 'qc_rotation']], on='subject_id', how='inner').copy()
...
#outer_res.to_csv('../../Results/predictions/nested_validation_categortical_motion_corrected.csv')
outer_res.to_csv('../../Results/predictions/new_inputFA_nested_validation_categorical_10reps.csv')
