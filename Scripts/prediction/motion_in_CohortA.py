...
###
#o_file = '../../Results/predictions/motion_correction_CohortA_results.txt'
o_file = '../../Results/predictions/motion_correction_CohortA_results_continuous.txt'
if os.path.exists(o_file):
    os.remove(o_file)
# ============================================ FUNCTIONS ========================== #
# ===                                                                           === #
def get_null_distribution(X, y, PMA_scan, corr_idx,qc_translation, qc_rotation, n_folds, mode, it=1000):
    ...
    if mode == 'cat':
        ...
        for i in range(it):
            ...
            y_true, y_pred = run_Kfold(X=X, y=y_shuff, PMA_scan=PMA_scan, qc_translation=qc_translation, qc_rotation=qc_rotation,
...
def get_repeated_Kfold(X, y, PMA_scan,qc_translation, qc_rotation, corr_idx, n_folds, mode, num_reps=100):   
    if mode == 'cat':
        res = {'auc' : [], 'acc' : [], 'spec' : [], 'sens' : []}
        for i in range(num_reps):
            X,y,PMA_scan,qc_translation, qc_rotation = unison_shuffled_copies(a=X, b=y, c=PMA_scan, d = qc_translation, e=qc_rotation)
            y_true, y_pred = run_Kfold(X=X, y=y, PMA_scan=PMA_scan, qc_translation=qc_translation, qc_rotation=qc_rotation,
                               corr_idx=for_corr_index, n_folds=n_folds, mode='cat')
            auc, acc, spec, sens = evaluate(y_true=y_true, y_pred=y_pred, mode='cat')
            res['auc'].append(auc)
            res['acc'].append(acc)
...
def run_Kfold(X, y, PMA_scan,qc_translation, qc_rotation, corr_idx, n_folds, mode):
	...
	for train_index, test_index in loo.split(X):
		...
		qc_translation_train, qc_translation_test = qc_translation[train_index], qc_translation[test_index]
		qc_rotation_train, qc_rotation_test = qc_rotation[train_index], qc_rotation[test_index]

		## process inputs 
		X_train, X_test = _pre_process(X_train=X_train, X_test=X_test, 
                                       PMA_train=PMA_train, PMA_test=PMA_test, qc_translation_train = qc_translation_train, qc_rotation_train = qc_rotation_train, qc_translation_test=qc_translation_test, 							qc_rotation_test=qc_rotation_test, 
...
def _pre_process(X_train, X_test, PMA_train, PMA_test, qc_translation_train, qc_rotation_train, qc_translation_test, qc_rotation_test, corr_idx, inflection=36):
	...
	if len(corr_idx) > 0 :
		...
		X_train, X_test = _correct_motion(X_train=X_train, X_test=X_test, 
                                      qc_translation_train = qc_translation_train, qc_rotation_train = qc_rotation_train, qc_translation_test=qc_translation_test, 							qc_rotation_test=qc_rotation_test,  corr_idx=corr_idx)
...
def _correct_motion(X_train, X_test, qc_translation_train, qc_rotation_train, qc_translation_test, 					qc_rotation_test, corr_idx):
	
	for idx in corr_idx:

	
		model = sm.OLS(X_train[:,idx], np.array([qc_translation_train, qc_rotation_train]).transpose()).fit()
        
		## correct train 
		yHat_train = model.predict(np.array([qc_translation_train, qc_rotation_train]).transpose()) 
		res_train = X_train[:,idx] - yHat_train
		
		## correct test 
		yHat_test = model.predict(np.array([qc_translation_test, qc_rotation_test]).transpose()) 
		res_test = X_test[:,idx] - yHat_test
...
outcomes = ['group_cat', 'Cognitive Score_cat','Language Score_cat','Motor Score_cat']
#outcome='group_cat'
...
for outcome in outcomes:
	...
	for key in inputs.keys():
		...
		qc_translation = sub_df['qc_translation'].values
		qc_rotation = sub_df['qc_rotation'].values
    
    		### RUN & EVALUATE 
		res = get_repeated_Kfold(X=X, y=y, PMA_scan=PMA_scan, qc_translation=qc_translation, qc_rotation=qc_rotation,
                             corr_idx=for_corr_index, n_folds=n_folds, 
                             mode='cat', num_reps=num_rep)
		null = get_null_distribution(X=X, y=y, PMA_scan=PMA_scan, qc_translation=qc_translation, qc_rotation=qc_rotation,
                             corr_idx=for_corr_index, n_folds=n_folds, 
                             mode='cat', it=it_num)
	...
	for random_parc_num in [128, 256, 512, 1024]:
		...
		random_df = pd.read_csv(os.path.join(path_to_random, 'random_parcellation_{}_diffusion_metric_median.csv'.format(random_parc_num)), index_col=0)
		random_df = pd.merge(random_df, sub_df[['subject_id', 'group_cat', 'PMA_scan','Cognitive Score_cat','Language Score_cat','Motor Score_cat', 'qc_translation', 'qc_rotation']], on='subject_id', how='inner')
		...
		for key in random_inputs.keys():
			...
			qc_translation = random_df['qc_translation'].values
			qc_rotation = random_df['qc_rotation'].values
...
for outcome in outcomes: 
	...
	for random_parc_num in [128, 256, 512, 1024]:
		path_to_random = '/neurospin/grip/external_databases/dHCP_CR_JD_2018/Projects/eLife_replication/DerivedData/extracted_metrics/'
		random_df = pd.read_csv(os.path.join(path_to_random, 'random_parcellation_{}_diffusion_metric_median.csv'.format(random_parc_num)), index_col=0)
		random_df = pd.merge(random_df, sub_df[['subject_id', 'PMA_scan','Cognitive Score','Language Score','Motor Score', 'qc_translation', 'qc_rotation', 'GA_birth']], on='subject_id', how='inner')
