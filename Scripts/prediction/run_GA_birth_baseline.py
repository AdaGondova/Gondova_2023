...
# ================================================================== RUN PREDICTION ==================================================================#
o_file = '../../Results/predictions/GA_birth_baseline_prediction_results.txt'
...
for cohort in [ 'C', 'D', 'E']:
	...
	with open(o_file, 'a') as the_file:
		...
		the_file.write('{}\n'.format(outcome))
		the_file.write('Inputs, RHO mean (std), Pval mean (std), MAE mean (std),R2 mean (std), Permuted RHO (p-val), Permuted MAE (p-val)\n')
	...
	print('Inputs, RHO mean (std), Pval mean (std), MAE mean (std),R2 mean (std), Permuted RHO (p-val), Permuted MAE (p-val)')
	...
	for random_parc_num in [128, 256, 512, 1024]:
		...
		random_df = pd.merge(random_df, sub_df[['subject_id', 'PMA_scan','GA_birth']], on='subject_id', how='inner')
