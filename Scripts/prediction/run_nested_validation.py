...
for inputs in ['ROIs age corrected', 128, 256, 512, 1024]:
	...
	if inputs == 'ROIs age corrected':
		...
		df = pd.merge(random_df, df[['subject_id', 'PMA_scan','Cognitive Score','Language Score','Motor Score', 'qc_translation', 'qc_rotation']], on='subject_id', how='inner').copy()
...
outer_res.to_csv('../../Results/predictions/nested_validation_motion_corrected.csv')
