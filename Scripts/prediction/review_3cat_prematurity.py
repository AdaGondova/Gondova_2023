...
df.loc[df['GA_birth'] < 32, 'group_cat'] = 2
...
def get_null_distribution(X, y, PMA_scan, corr_idx, n_folds, mode, it=1000):
    ...
    if mode == 'cat':
        ...
        for i in range(it):
            ...
            auc = evaluate(y_true=y_true, y_pred=y_pred, mode='cat')
            res['auc'].append(auc)
...
def evaluate(y_true, y_pred, mode):
	...
	if mode == 'cat':
		#y = np.transpose([pred[:,1] for pred in y_pred])
		#roc = roc_auc_score(y_true, y, average=None)
		#print(roc)
...
def run_model(X_train, X_test, y_train, y_test, mode): 
    ## mode is 'cat' or 'cont'
    if mode == 'cat':
        clf = SVC( C=9, kernel='linear', decision_function_shape='ovr', probability=True)
...
# ================================================================== RUN PREDICTION ==================================================================#
o_file = '../../Results/predictions/review_3cat_prematurity_prediction_results_minAD.txt'
...
for cohort in ['A']:
	...
	with open(o_file, 'a') as the_file:
		...
		the_file.write('{}\n'.format(outcome))
		the_file.write('Inputs, AUC mean (std)\n')
	...
	for key in inputs.keys():
		...
		auc_p = np.sum(null['auc']>=res['auc'][0]) / len(null['auc'])

		out_message = '{}, {:.3f}({:.3f}),{:.3f}({:.6f})\n'.format(
                        key,
	...
	for random_parc_num in [128, 256, 512, 1024]:
		...
		random_df = pd.merge(random_df, sub_df[['subject_id', 'group_cat', 'PMA_scan','Cognitive Score_cat','Language Score_cat','Motor Score_cat']], on='subject_id', how='inner')
